...
@registry.tool(
    tags=["memory", "read"],
    annotations=Hints(read_only=False, destructive=False, idempotent=True, open_world=True),
)
def vault_promotion_queue(
    ...
    Worklist, four buckets (issue #92): 'debt' (tagged promotion-debt),
    'drift' (unresolved memory_drift rows), 'dead_handoffs' (stale context
    memories that read as handoffs; 'open-thread' tag exempts), 'uncovered'
    (durable memories the judgement model says no note records yet, each with
    its 0-3 'coverage' score and nearest note). Coverage verdicts are cached,
    so a memory is judged once until it or its nearest notes change;
    'uncovered_pending' counts memories the judge did not answer this time.
    A downstream agent writes the notes and clears entries via
    vault_update_memory/vault_forget.

    Args:
        workspace: Optional vault subdirectory scope
        handoff_age_days: Grace window before a handoff counts as dead (default 14)
        uncovered_limit: Newest durable memories checked for coverage (default 200)
    """
