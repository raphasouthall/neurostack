...
- Hybrid search now runs keyword and embedding search as separate top-50 lists and merges them by reciprocal rank (k = 60), replacing the keyword-only candidate pool (#266). On 30 paraphrased questions over a 266-note folder, the right note reached the top 10 in 97% of queries, up from 57%, and MRR with rerank rose from 0.517 to 0.828. On 28 past clicked searches, MRR without rerank rose from 0.479 to 0.618 and top-10 fell from 96% to 86%.
...
- **#257 — shadcn components in the dashboard.** Every page now renders its buttons, badges, cards, inputs, selects, switches, sliders, tables, tabs, alerts, skeletons, tooltips and the mobile menu through `ui/static/components/ui.js`, a no-build Preact port of shadcn/ui (MIT). CSS variables use the shadcn names (`--background`, `--primary`, `--muted-foreground`, `--ring`, `--radius` and the rest), so a theme from ui.shadcn.com/themes can be pasted into `styles.css`. Keyboard support follows shadcn: arrow keys move between tabs, Space toggles a switch, and the mobile menu traps focus, closes on Escape and hands focus back. The look is unchanged.
- **#255 — card effects, Settings page, fits every screen.** Dashboard card grids get the React Bits MagicBento effects (stars, cursor spotlight, border glow, tilt, magnetism, click ripple, text auto-hide), ported to the Web Animations API with no gsap. A new Settings page exposes every effect option plus the theme, applies changes live and remembers them. Effects turn off on screens 768px wide or less, under reduced motion, or when disabled. Every page fits from 375px to 1920px with no sideways scroll: the sidebar becomes a menu on phones, grids reflow from 4 columns to 1, and wide tables scroll inside their card.
...
### Added
- **`neurostack agent --prompt-file PATH|-` and `--mode`.** Runs your own prompt on the bundled Pi agent, so personal jobs (vault heal, a morning briefing) stay out of the package; `-` reads the prompt from stdin, which lets a script on another host stream it over ssh. `--mode X` appends `## RUN MODE / MODE=X` to the prompt. Two new agent tools: `graph_analysis` (`neurostack --json graph-analysis`) and `note_summary` (`neurostack --json summary`).
- **`agent_base_url`** (env `NEUROSTACK_AGENT_BASE_URL`) points `neurostack agent` at a compatible proxy instead of the provider's own endpoint. With `agent_provider = "anthropic"`, `agent_model = "claude-sonnet-5"` and a CLIProxyAPI URL, promotion runs on a Claude subscription login rather than OpenRouter credits.
- **#217 — `neurostack agent <job>`, a bundled Pi agent.** Jobs that need tools and reasoning run on the Pi SDK (`@earendil-works/pi-coding-agent` 0.87.1) instead of a harness CLI. The runner (`src/neurostack/agent/run.mjs`) and its `package.json` ship as package data; the Node dependencies install once into `~/.cache/neurostack/agent` and reinstall only when `package.json` changes. Memory operations are custom tools calling the CLI directly (`promotion_queue`, `memory_search`, `vault_search`, `memory_update`, `memory_forget`), so a run needs neither the MCP server nor a harness login; built-in read/edit/write/bash/grep/find/ls work in the vault clone. First job: `promotion`. New config `agent_provider` (default `openrouter`), `agent_model` (default `anthropic/claude-sonnet-5`), `agent_api_key` (falls back to `judge_api_key`). Needs Node 22.19+.
- **#215 — the promotion queue's uncovered bucket is judged, not thresholded.** A durable memory used to count as uncovered when its nearest note chunk scored below 0.55 similarity. After the switch to qwen3-embedding-8b nothing scored below 0.566, so the bucket sat empty. Now similarity only picks the evidence, the three nearest notes, and the judgement model scores coverage 0-3 against them; below 1.5 is uncovered. Each uncovered entry carries `coverage`. Verdicts are cached in `memory_coverage` under a hash of the memory text, the evidence notes' content hashes and the question, so a memory is asked about once until one of those changes. A memory the judge does not answer is left out and counted in the new top-level `uncovered_pending`. `vault_promotion_queue(uncovered_sim_floor=…)` and `neurostack promote --sim-floor` are gone.
...
- v27 → v28: `memory_coverage` table (#215).
